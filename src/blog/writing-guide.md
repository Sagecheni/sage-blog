...
**默认行为**：≥ 8 行的代码块会**整段折叠**（摘要：「已折叠 N 行 · 点击展开」），展开后仍可再折上——大段 docker-compose / nginx 不再把正文撑到要滑半天。不足 8 行的短 snippet 保持展开。

代码块还支持文件名标题、行高亮、diff 与手写折叠，写在开头反引号之后：
...
- `collapse={8-15}` —— 只折指定行段（会覆盖「整段自动折」）
- `nocollapse` —— 退出自动折叠，强制整段展开（长配置也一直摊开）
...
- **悬停双链会浮出目标文章的预览卡**（标题/描述/日期），不点进去就知道链接背后是什么
